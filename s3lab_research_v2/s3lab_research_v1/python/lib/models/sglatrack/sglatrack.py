...
class sglatrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, aux_loss=False, head_type="CORNER",
                 feat_len_t=64,
                 orr_enable=False, orr_random_mask=False, orr_block_sz=16,
                 orr_mask_ratio=0.3, orr_gaussian_sigma=64):
...
def build_sglatrack(cfg, training=True):
    ...
    # 僅檔名／路徑為 .pth.tar（訓練儲存的整模組）時才略過 pretrained_models；勿用子字串
    # "sglatrack"，否則 vit_clip_*_sglatrack.pth 會被誤判並 torch.load(檔名) 失敗。
    _pf = str(cfg.MODEL.PRETRAIN_FILE or '')
    if _pf and (not _pf.endswith('.pth.tar')) and training:
    ...
    if _pf.endswith('.pth.tar') and training:
