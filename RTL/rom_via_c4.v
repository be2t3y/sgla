...
//       
//       Creation Date:  Fri Jul 28 13:14:40 2023
//       Version: 	r4p1
//
...
`ifdef POWER_PINS
module rom_via_c4 (VDDE, VSSE, CENY, AY, Q, CLK, CEN, A, EMA, TEN, BEN, TCEN, TA, TQ,
    PGEN, STOV, KEN);
`else
module rom_via_c4 (CENY, AY, Q, CLK, CEN, A, EMA, TEN, BEN, TCEN, TA, TQ, PGEN, STOV,
    KEN);
`endif
...
`endif
  ...
  reg NOT_CEN, NOT_A12, NOT_A11, NOT_A10, NOT_A9, NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4;
  reg NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_TEN, NOT_TCEN;
  reg NOT_TA12, NOT_TA11, NOT_TA10, NOT_TA9, NOT_TA8, NOT_TA7, NOT_TA6, NOT_TA5, NOT_TA4;
  reg NOT_TA3, NOT_TA2, NOT_TA1, NOT_TA0, NOT_PGEN, NOT_STOV, NOT_KEN;
 ...
 wire  CLK_;
  ...
  initial begin
    $readmemb("rom_via_c4_verilog.rcf", mem);
  end
...
`endif
 ...
 wire  CLK_;
  ...
  buf B34(A_[11], A[11]);
  buf B35(A_[12], A[12]);
  buf B36(EMA_[0], EMA[0]);
  buf B37(EMA_[1], EMA[1]);
  buf B38(EMA_[2], EMA[2]);
  buf B39(TEN_, TEN);
  buf B40(BEN_, BEN);
  buf B41(TCEN_, TCEN);
  buf B42(TA_[0], TA[0]);
  ...
  buf B50(TA_[8], TA[8]);
  buf B51(TA_[9], TA[9]);
  buf B52(TA_[10], TA[10]);
  buf B53(TA_[11], TA[11]);
  buf B54(TA_[12], TA[12]);
  buf B55(TQ_[0], TQ[0]);
  buf B56(TQ_[1], TQ[1]);
  buf B57(TQ_[2], TQ[2]);
  buf B58(TQ_[3], TQ[3]);
  ...
  assign AY_ = TEN_ ? A_ : TA_;
   `ifdef ARM_FAULT_MODELING
     rom_via_c4_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rom_via_c4_error_injection (Q_out, Q_in, CLK, A, CEN, BEN, TQ);
   output [6:0] Q_out;
   input [6:0] Q_in;
   input CLK;
   input [12:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.READONLY_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(13'd3903,3'd4,2'd1,2'd0);
   `endif
end
   task add_fault;
