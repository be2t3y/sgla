...
//       
//       Creation Date:  Fri Jul 28 12:42:58 2023
//       Version: 	r8p2
//
...
`ifdef POWER_PINS
module img_rf1_3 (VDDCE, VDDPE, VSSE, CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA,
    EMAW, EMAS, TEN, BEN, TCEN, TWEN, TA, TD, TQ, RET1N, RET2N, RETP, PGEN, STOV);
`else
module img_rf1_3 (CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, TEN,
 ...
 wire  CLK_;
  ...
  assign DY_ = retention === 1'bx ? {8{1'bx}} : (retention === 1'b1 ? {8{1'b1}} : (TEN_ ? D_ : TD_));
   `ifdef ARM_FAULT_MODELING
     img_rf1_3_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module img_rf1_3_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [7:0] Q_out;
   input [7:0] Q_in;
   input CLK;
   input [10:0] A;
