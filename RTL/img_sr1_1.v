...
//       
//       Creation Date:  Fri Jul 28 12:57:42 2023
//       Version: 	r11p2
//
...
`ifdef POWER_PINS
module img_sr1_1 (VDDCE, VDDPE, VSSE, CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA,
    EMAW, EMAS, TEN, BEN, TCEN, TWEN, TA, TD, TQ, RET1N, RET2N, RETP, PGEN, STOV);
`else
module img_sr1_1 (CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, TEN,
...
`endif
  ...
  reg NOT_A5, NOT_A4, NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D7, NOT_D6, NOT_D5, NOT_D4;
  reg NOT_D3, NOT_D2, NOT_D1, NOT_D0, NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0;
  reg NOT_EMAS, NOT_TEN, NOT_TCEN, NOT_TWEN, NOT_TA12, NOT_TA11, NOT_TA10, NOT_TA9;
  reg NOT_TA8, NOT_TA7, NOT_TA6, NOT_TA5, NOT_TA4, NOT_TA3, NOT_TA2, NOT_TA1, NOT_TA0;
  reg NOT_TD7, NOT_TD6, NOT_TD5, NOT_TD4, NOT_TD3, NOT_TD2, NOT_TD1, NOT_TD0, NOT_RET1N;
  reg NOT_RET2N, NOT_RETP, NOT_PGEN, NOT_STOV;
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_DP AY_ = retention === 1'bx ? {13{1'bx}} : (retention === 1'b1 ? {13{1'b1}} : (TEN_ ? A_ : TA_));
...
`endif
 ...
 wire  CLK_;
  ...
  buf B53(D_[6], D[6]);
  buf B54(D_[7], D[7]);
  buf B55(EMA_[0], EMA[0]);
  buf B56(EMA_[1], EMA[1]);
  buf B57(EMA_[2], EMA[2]);
  buf B58(EMAW_[0], EMAW[0]);
  buf B59(EMAW_[1], EMAW[1]);
  buf B60(EMAS_, EMAS);
  buf B61(TEN_, TEN);
  buf B62(BEN_, BEN);
  buf B63(TCEN_, TCEN);
  ...
  buf B92(TQ_[6], TQ[6]);
  buf B93(TQ_[7], TQ[7]);
  buf B94(RET1N_, RET1N);
  buf B95(RET2N_, RET2N);
  buf B96(RETP_, RETP);
  buf B97(PGEN_, PGEN);
  buf B98(STOV_, STOV);
  ...
  assign AY_ = retention === 1'bx ? {13{1'bx}} : (retention === 1'b1 ? {13{1'b1}} : (TEN_ ? A_ : TA_));
  assign DY_ = retention === 1'bx ? {8{1'bx}} : (retention === 1'b1 ? {8{1'b1}} : (TEN_ ? D_ : TD_));
   `ifdef ARM_FAULT_MODELING
     img_sr1_1_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module img_sr1_1_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [7:0] Q_out;
   input [7:0] Q_in;
   input CLK;
   input [12:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(13'd476,3'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
