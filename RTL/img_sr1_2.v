...
//       
//       Creation Date:  Fri Jun 30 10:08:02 2023
//       Version: 	r11p2
//
...
`ifdef POWER_PINS
module img_sr1_2 (VDDCE, VDDPE, VSSE, CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA,
    EMAW, EMAS, TEN, BEN, TCEN, TWEN, TA, TD, TQ, RET1N, RET2N, RETP, PGEN, STOV);
`else
module img_sr1_2 (CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, TEN,
...
`endif
  ...
  reg NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D7, NOT_D6, NOT_D5, NOT_D4, NOT_D3, NOT_D2;
  reg NOT_D1, NOT_D0, NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0, NOT_EMAS;
  reg NOT_TEN, NOT_TCEN, NOT_TWEN, NOT_TA11, NOT_TA10, NOT_TA9, NOT_TA8, NOT_TA7, NOT_TA6;
  reg NOT_TA5, NOT_TA4, NOT_TA3, NOT_TA2, NOT_TA1, NOT_TA0, NOT_TD7, NOT_TD6, NOT_TD5;
  reg NOT_TD4, NOT_TD3, NOT_TD2, NOT_TD1, NOT_TD0, NOT_RET1N, NOT_RET2N, NOT_RETP;
  reg NOT_PGEN, NOT_STOV;
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_DP AY_ = retention === 1'bx ? {12{1'bx}} : (retention === 1'b1 ? {12{1'b1}} : (TEN_ ? A_ : TA_));
	...
	dump_file_desc = $fopen(filename_dump);
     if (CEN_ === 1'b1) begin
      ...
      row = mem[row_address];
        ...
        Q_int = {shifted_readLatch0[28], shifted_readLatch0[24], shifted_readLatch0[20],
...
`endif
 ...
 wire  CLK_;
  ...
  buf B51(D_[6], D[6]);
  buf B52(D_[7], D[7]);
  buf B53(EMA_[0], EMA[0]);
  buf B54(EMA_[1], EMA[1]);
  buf B55(EMA_[2], EMA[2]);
  buf B56(EMAW_[0], EMAW[0]);
  buf B57(EMAW_[1], EMAW[1]);
  buf B58(EMAS_, EMAS);
  buf B59(TEN_, TEN);
  buf B60(BEN_, BEN);
  buf B61(TCEN_, TCEN);
  ...
  buf B71(TA_[8], TA[8]);
  buf B72(TA_[9], TA[9]);
  buf B73(TA_[10], TA[10]);
  buf B74(TA_[11], TA[11]);
  buf B75(TD_[0], TD[0]);
  buf B76(TD_[1], TD[1]);
  buf B77(TD_[2], TD[2]);
  buf B78(TD_[3], TD[3]);
  ...
  buf B89(TQ_[6], TQ[6]);
  buf B90(TQ_[7], TQ[7]);
  buf B91(RET1N_, RET1N);
  buf B92(RET2N_, RET2N);
  buf B93(RETP_, RETP);
  buf B94(PGEN_, PGEN);
  buf B95(STOV_, STOV);
  ...
  assign AY_ = retention === 1'bx ? {12{1'bx}} : (retention === 1'b1 ? {12{1'b1}} : (TEN_ ? A_ : TA_));
  assign DY_ = retention === 1'bx ? {8{1'bx}} : (retention === 1'b1 ? {8{1'b1}} : (TEN_ ? D_ : TD_));
   `ifdef ARM_FAULT_MODELING
     img_sr1_2_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
  ...
  specify
    ...
    if (PGEN == 1'b0 && BEN == 1'b1 && STOV == 1'b0 && ((TEN == 1'b1 && WEN == 1'b0) || (TEN == 1'b0 && TWEN == 1'b0)))
       (posedge CLK => (Q[7] : 1'b0)) = (1.000, 1.000);
    if (PGEN == 1'b0 && BEN == 1'b1 && STOV == 1'b0 && ((TEN == 1'b1 && WEN == 1'b0) || (TEN == 1'b0 && TWEN == 1'b0)))
       (posedge CLK => (Q[6] : 1'b0)) = (1.000, 1.000);
    if (PGEN == 1'b0 && BEN == 1'b1 && STOV == 1'b0 && ((TEN == 1'b1 && WEN == 1'b0) || (TEN == 1'b0 && TWEN == 1'b0)))
...
`endif
`timescale 1ns/1ps
module img_sr1_2_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [7:0] Q_out;
   input [7:0] Q_in;
   input CLK;
   input [11:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(12'd3718,3'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
