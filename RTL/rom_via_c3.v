...
//       
//       Creation Date:  Fri Jul 28 13:14:11 2023
//       Version: 	r4p1
//
...
`ifdef POWER_PINS
module rom_via_c3 (VDDE, VSSE, CENY, AY, Q, CLK, CEN, A, EMA, TEN, BEN, TCEN, TA, TQ,
    PGEN, STOV, KEN);
`else
module rom_via_c3 (CENY, AY, Q, CLK, CEN, A, EMA, TEN, BEN, TCEN, TA, TQ, PGEN, STOV,
    KEN);
`endif
...
`endif
 ...
 wire  CLK_;
  ...
  initial begin
    $readmemb("rom_via_c3_verilog.rcf", mem);
  end
 ...
 wire  CLK_;
  ...
  assign AY_ = TEN_ ? A_ : TA_;
   `ifdef ARM_FAULT_MODELING
     rom_via_c3_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rom_via_c3_error_injection (Q_out, Q_in, CLK, A, CEN, BEN, TQ);
   output [6:0] Q_out;
   input [6:0] Q_in;
   input CLK;
   input [10:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.READONLY_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(11'd199,3'd4,2'd1,2'd0);
   `endif
end
   task add_fault;
