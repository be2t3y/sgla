...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    if torch.isnan(anno_bb).any():
        if dataset in ('uav', 'uavtrack', 'uavtrack112', 'uav123', 'uav123_10fps', 'uav123_test'):
            pass
        else:
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results (use seq.dataset for path - tracker saves to results_dir/seq.dataset/seq.name.txt)
            dataset_subdir = getattr(seq, 'dataset', report_name)
            base_results_path = '{}/{}/{}'.format(trk.results_dir, dataset_subdir, seq.name)
            results_path = '{}.txt'.format(base_results_path)

            time_file = os.path.join(trk.results_dir, dataset_subdir, '%s_time.txt' % seq.name)
