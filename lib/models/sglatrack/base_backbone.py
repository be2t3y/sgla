...
class BaseBackbone(nn.Module):
    ...
    def _is_t2t_backbone(self):
        return hasattr(self.patch_embed, 'soft_split0')
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        if self._is_t2t_backbone():
            print('T2T backbone: keeping T2T_module, initializing pos_embed for tracking resolution.')
            H_t, W_t = template_size
            H_s, W_s = search_size
            num_patches_z = (H_t // 16) * (W_t // 16)
            num_patches_x = (H_s // 16) * (W_s // 16)
            template_patch_pos_embed = torch.zeros(1, num_patches_z, self.embed_dim)
            search_patch_pos_embed = torch.zeros(1, num_patches_x, self.embed_dim)
            trunc_normal_(template_patch_pos_embed, std=.02)
            trunc_normal_(search_patch_pos_embed, std=.02)
    ...
    def forward_test(self, z, x):
        ...
        # Compute cos_tensor for consistency with training mode
        with torch.no_grad():
            cos_tensor = torch.ones(B, 12-1-start_layer, device=x.device)
            for i, blk in enumerate(self.blocks):
                if i > start_layer:
