# AQATrack-style query decoder (spatial-temporal memory). Adapted for SGLATrack integration.
import copy
...
def build_transformer_dec(cfg, hidden_dim):
    dec_cfg = cfg.MODEL.TRANSFORMER_DEC
    dim_ff = getattr(dec_cfg, 'DIM_FEEDFORWARD', None)
    if dim_ff is None or dim_ff == '':
        dim_ff = max(hidden_dim * 4, 512)
    else:
        dim_ff = int(dim_ff)
    return Transformer_dec(
        d_model=hidden_dim,
        dropout=dec_cfg.DROPOUT,
        nhead=dec_cfg.NHEADS,
        dim_feedforward=dim_ff,
        num_encoder_layers=dec_cfg.ENC_LAYERS,
        num_decoder_layers=dec_cfg.DEC_LAYERS,
        normalize_before=dec_cfg.PRE_NORM,
        return_intermediate_dec=False,
        divide_norm=dec_cfg.DIVIDE_NORM
    )
