        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    # 存檔時機：每 5 epoch 存一次，避免中斷後沒有 checkpoint；最後幾 epoch 也會存
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)
                    save_interval = getattr(self.settings, "save_epoch_interval", 5)
                    if epoch > (max_epochs - 1) or save_every_epoch or epoch % save_interval == 0 or epoch % 40 == 0 or epoch > (max_epochs - 5):
