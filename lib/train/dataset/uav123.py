"""
UAV123 training dataset.
Adapted from uav123_10fpsdataset - uses data_seq/UAV123/ (full fps).
Supports split='train', 'val', or None (all sequences).
Requires train_split.txt, val_split.txt in root for split mode.
"""
import os
import random
...
# Full fps UAV123 sequence info (path data_seq/UAV123, frame scale 3x from 10fps)
def _get_uav123_sequence_info():
    # 10fps info: name, path_suffix (folder), startFrame, endFrame
    # Full fps: multiply endFrame by 3, path uses data_seq/UAV123
    infos_10fps = [
    ...
    # Scale to full fps: ~3x frames
    out = []
    for name, folder, start, end in infos_10fps:
        end_full = min(int(end * 3), 99999)  # cap for safety
        out.append({
            'name': name,
            'path': f'data_seq/UAV123/{folder}',
            'startFrame': start,
            'endFrame': end_full,
            'nz': 6,
            'ext': 'jpg',
            'anno_path': f'anno/UAV123/{name}.txt',
            'object_class': 'object',
        })
    return out
...
class UAV123(BaseVideoDataset):
    ...
    def _build_sequence_list(self):
        if self.split is not None:
            ...
            if os.path.exists(split_file):
                ...
                if names:
                    print(f'UAV123: Loaded {len(names)} sequences from {self.split}_split.txt')
                    return names
            print(f'Warning: {split_file} not found, using all sequences.')
    ...
    def _build_seq_per_class(self):
        ...
        for i, name in enumerate(self.sequence_list):
            cls = name.split('_')[0].split('0')[0] or name[:2]
            if cls not in d:
                d[cls] = []
            d[cls].append(i)
        return d
    ...
    def _read_bb_anno(self, seq_name):
        path = os.path.join(self.root, 'anno', 'UAV123', seq_name + '.txt')
    ...
    def _get_frame_path(self, seq_id, frame_id):
        ...
        # Frame in folder: startFrame + frame_id (0-based)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        meta = OrderedDict({
            'object_class_name': info.get('object_class', 'object'),
